...
```cpp
template <class T>
using Result = std::variant<T, Error>;   // Error = { int code; std::string msg; }
...
if (auto* err = std::get_if<Error>(&r)) {
    bus.push(NetworkError{ Service::Stt, err->msg });
    return;
}
...
### 5.1 `Error` luôn mang **cả `code` lẫn `msg`** — chúng phục vụ hai người đọc khác nhau

| Trường | Ai đọc | Dùng để |
|--------|--------|---------|
| `code` | **chương trình** | rẽ nhánh xử lý: `NotFound` → fallback, `ParseError` → thoát |
| `msg`  | **con người** | log / hiển thị LCD: `"missing field 'host'"` |

Hai trường **không thay thế nhau**. Một `code` không tái tạo được ngữ cảnh người-đọc (`ParseError` không nói *field nào* sai); một `msg` không rẽ nhánh tự động được. Vì vậy:

> **Quy ước: hàm fallible trả `Result<T>`; nhánh lỗi luôn là `Error{ code, msg }` đầy đủ — luôn có `msg`, kể cả ngắn.** Lúc debug trên board, log có chữ nhanh hơn tra bảng code.

**Đừng** trả về bare error code (`int`/`enum`) thay cho `Result`, trừ khi **đồng thời** ba điều kiện: (1) hàm `void`/không có payload trả về, (2) caller *chỉ* rẽ nhánh, không bao giờ hiển thị message, (3) nằm trong hot loop cần tránh cấp phát `std::string`. Ngoài ba điều kiện đó → full `Result`. Lý do "string tốn kém" thường không đứng vững: `Result<T>` thành công giữ `T`, **không** dựng string; `msg` chỉ sinh ra trên nhánh fail vốn hiếm.

Hệ quả phụ: hàm có giá trị trả về (vd `loadConfig` → `AppConfig`) **buộc** phải dùng `Result<T>` để chở payload — bare code không chở được. Câu hỏi gần như tự trả lời.

### 5.2 Mã lỗi: `enum class` riêng từng subsystem, cast ở biên

`Error::code` ở `Types.hpp` (tầng common) cố tình là `int` — mẫu số chung để mọi subsystem nhét code riêng vào mà **không** phụ thuộc nhau. **Không** đổi `code` thành enum của một subsystem cụ thể: làm thế thì `Types.hpp` (tầng dưới cùng) phải biết về Config/Network → rò rỉ ngược tầng.

Thay vào đó, mỗi subsystem định nghĩa `enum class … : int` **cạnh nơi dùng**, đánh số **bắt đầu từ 1** (tránh `Error` mang `code == 0` dễ nhầm với "OK"), và `static_cast` tại biên khi *tạo* và khi *so sánh*:

```cpp
// trong Config.hpp — không nằm ở Types.hpp
enum class ConfigError : int { NotFound = 1, ParseError, MissingField };

return Error{ static_cast<int>(ConfigError::ParseError), "invalid config: " + std::string(e.what()) };
//  caller:  if (err.code == static_cast<int>(ConfigError::NotFound)) { ... }
```

Chỉ tạo enum khi caller **thật sự rẽ nhánh** theo loại lỗi; nếu mọi lỗi đều xử lý như nhau (log → ERROR) thì `msg` là đủ, đừng dựng enum chết. Và một khi đã có enum, **không** viết số trần (`Error{1, …}`) ở bất kỳ đâu khác — luôn `static_cast` từ enum.

### 5.3 Ai log? — callee **trả** lỗi, caller **quyết** log

Hàm fallible **trả** `Error` về cho caller; **caller** quyết định log/hiển thị/đẩy event. Đừng để hàm "load/transcribe/…" vừa làm việc vừa tự log — đó là trộn trách nhiệm và khiến hàm khó test (phụ thuộc Logger). Full `Result{code,msg}` chính là thứ cho phép tách sạch: caller có đủ thông tin để tự log mà không cần callee làm hộ.
...
spdlog::error("STT unreachable: {}", err.msg);
```
...
- [ ] Hàm lỗi trả `Result<T>` với `Error{code, msg}` đầy đủ (không bare code), không throw qua biên thread
- [ ] Mã lỗi dùng `enum class` của subsystem (cast ở biên), không số trần; callee trả lỗi, caller log
- [ ] `const` ở đâu có thể
